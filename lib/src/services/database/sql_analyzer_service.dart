...
class ExecutionPlanNode {
  ...
  final List<ExecutionPlanNode> children;
  final double cost;
  final int rows;

  const ExecutionPlanNode({
  ...
  factory ExecutionPlanNode.fromJson(Map<String, dynamic> json) {
    return ExecutionPlanNode(
      ...
      children: (json['children'] as List?)
          ?.map((e) => ExecutionPlanNode.fromJson(e as Map<String, dynamic>))
          .toList() ?? [],
...
/// 查询性能结果
class SqlQueryPerformanceResult {
  final List<SqlRecommendation> recommendations;
  final Map<String, dynamic> metrics;
  final bool hasIssues;
  
  SqlQueryPerformanceResult({
...
/// 推荐优先级
enum SqlRecommendationPriority {
  low,
  medium,
  high,
}
...
class SqlRecommendation {
  final SqlRecommendationType type;
  final String message;
  final SqlRecommendationPriority priority;
  final String? code;
